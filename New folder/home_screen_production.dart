...
import '../customer/cuisine_results_screen.dart';
...
Widget _buildHomeNetworkImage(
  ...
  if (resolvedUrl.isEmpty) return errorWidget ?? const SizedBox.shrink();
...
class _CustomerHomeScreenProductionState
    extends State<CustomerHomeScreenProduction> with RouteAware {
  ...
  void dispose() {
    ...
    if (customerId != null) {
      WebSocketService().removeCustomerHandler(customerId);
    }
    if (_isRouteAwareSubscribed) {
  ...
  Future<void> _initializeCustomerRealtime() async {
    for (var attempt = 0; attempt < 10 && mounted; attempt++) {
      ...
      if (user != null) {
        ...
        await WebSocketService().initCustomer(
          user.id,
  ...
  Future<void> _applySelectedLocation({
    ...
    await _locationService.saveLocation(city, latitude, longitude);
    if (!mounted) return;
...
class _CustomerHomeFeedState extends State<_CustomerHomeFeed> {
    ...
    try {
      if (await _isOfflineNow()) {
        ...
        setState(() {
          ...
          _blockingState = _HomeBlockingState.offline;
        });
        return;
      }
      ...
      Future<dynamic> restaurantFeedRequest() {
        ...
        return _api.get(
          ...
          queryParams: <String, dynamic>{
            'lat': lat,
            'lng': lng,
            if (!widget.showDiningMode) 'radius': 15,
          },
        );
      }

      final results = await Future.wait<dynamic>([
        _safeHomeGet(ApiConstants.popularCuisines),
        _safeHomeGet(ApiConstants.homeSections),
      ]);
      ...
      setState(() {
        ...
        _categories = nextCategories.isNotEmpty || _categories.isEmpty
            ? nextCategories
            : _categories;
        _homeSections = nextHomeSections;
      ...
      unawaited(_loadDeferredHomeContent(nextHomeSections));
  ...
  Future<void> _loadDeferredHomeContent(
    ...
    try {
      final results = await Future.wait<dynamic>([
        ...
        _safeHomeGet('${ApiConstants.bannersByType}/home'),
        _safeHomeGet(ApiConstants.activeOffers),
      ]);
  ...
  Future<void> _refresh() async {
    ...
    await Future.wait([_loadHomeData(), _loadNotificationCount()]);
    if (!mounted) return;
    setState(() {
      _isRefreshing = false;
    });
  ...
  Future<void> _loadPopularDishes(
    ...
    final seen = <String>{};
    final restaurantBatch = restaurants.take(6).toList(growable: false);
  ...
  void _precacheHomeImages({
    ...
    for (final item in banners.take(8)) {
      add(_resolveImageUrl(
        item,
        const ['image_url', 'image', 'banner_image', 'hero_image', 'photo'],
      ));
    }
    for (final item in popupCampaigns.take(3)) {
      add(_campaignHeroImageUrl(item));
      add(_campaignLogoUrl(item));
    }
    for (final item in categories.take(12)) {
      add(_resolveImageUrl(item, const ['image_url', 'image', 'icon']));
    }
    for (final restaurant in restaurants.take(12)) {
      add(_resolveImageUrl(
        restaurant,
        const ['logo_image', 'logo', 'image', 'banner_image'],
      ));
    }
    for (final dish in dishes.take(10)) {
      add(dish.imageUrl);
    }
    for (final section in sections.take(10)) {
      final items = section['items'];
    ...
    return <Map<String, dynamic>>[
      if (dishes.isNotEmpty)
        <String, dynamic>{
          ...
          'resolved_items': dishes.take(12).toList(),
        },
      ...
      if (restaurants.isNotEmpty)
        <String, dynamic>{
          ...
          'resolved_items': restaurants.take(12).toList(),
        },
      if (dishes.isNotEmpty)
        <String, dynamic>{
          'type': 'popular_dishes',
  ...
  double _menuItemPrice(Map<String, dynamic> item) {
    for (final value in <dynamic>[
      ...
      final parsed = _parseDouble(value, fallback: 0);
  ...
  void _openCategory(dynamic category) {
    ...
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => CuisineResultsScreen(
          title: title,
          cuisineId: cuisineId > 0 ? cuisineId : null,
        ),
      ),
    );
  }
              ...
              child: ListView.separated(
                ...
                itemBuilder: (context, index) => _DishPreviewCard(
                  ...
                  onTap: () => _openRestaurant(
                    <String, dynamic>{
                      'id': dishItems[index].restaurantId,
          ...
          children: <Widget>[
            ...
            SizedBox(
              ...
              child: GridView.builder(
                ...
                itemCount: min(recommendedDishes.length, 12),
                itemBuilder: (context, index) => _RecommendedDishCard(
                  dish: recommendedDishes[index],
                  onTap: () => _openRestaurant(
                    <String, dynamic>{
                      'id': recommendedDishes[index].restaurantId,
                      'name': recommendedDishes[index].restaurantName,
                    },
                  ),
                  onAdd: () => _addDishToCart(recommendedDishes[index]),
                ),
              ),
            ),
          ],
          ...
          children: <Widget>[
            ...
            SizedBox(
              ...
              child: ListView.separated(
                ...
                separatorBuilder: (_, __) => const SizedBox(width: 14),
                itemBuilder: (context, index) => _DishPreviewCard(
                  dish: items[index] as _HomeDishCardData,
                  onTap: () => _openRestaurant(
                    <String, dynamic>{
                      'id': (items[index] as _HomeDishCardData).restaurantId,
                      'name':
                          (items[index] as _HomeDishCardData).restaurantName,
                    },
                  ),
                  onAdd: () =>
                      _addDishToCart(items[index] as _HomeDishCardData),
                ),
              ),
            ),
          ],
                  ...
                  child: ListView.separated(
                    ...
                    itemBuilder: (context, index) => _DishListTile(
                      ...
                      onTap: () {
                        Navigator.pop(context);
                        _openRestaurant(
                          <String, dynamic>{'id': dishes[index].restaurantId},
                        );
                      },
                    ),
                    separatorBuilder: (_, __) => const SizedBox(height: 12),
  ...
  void _addDishToCart(_HomeDishCardData dish) {
    ...
    if (item == null || restaurant == null) {
      _openRestaurant(<String, dynamic>{'id': dish.restaurantId});
      return;
    }
...
class _HomeHeroPinnedDelegate extends SliverPersistentHeaderDelegate {
  ...
  double get _panelHeight => categories.isNotEmpty ? 206 : 104;
  ...
  @override
  double get maxExtent => topInset + 500;
  ...
  Widget build(
    ...
    final showExpandedCuisine = categories.isNotEmpty && progress < 0.28;
        ...
        children: <Widget>[
          ...
          Positioned(
            ...
            child: _PinnedSearchCuisineStrip(
              categories: showExpandedCuisine ? const <dynamic>[] : categories,
          ...
          if (showExpandedCuisine)
            Positioned(
              ...
              child: Opacity(
                opacity: ((0.28 - progress) / 0.28).clamp(0.0, 1.0),
        ...
        SizedBox(
          ...
          child: PageView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final hasText = eyebrow.isNotEmpty ||
                  headline.isNotEmpty ||
                  subtitle.isNotEmpty ||
                  cta.isNotEmpty;
                ...
                child: ClipRRect(
                  ...
                  child: Container(
                    ...
                    child: Stack(
                      fit: StackFit.expand,
                      children: <Widget>[
                        _buildBannerMedia(mediaUrl, fit: BoxFit.cover),
                        if (hasText)
                          DecoratedBox(
                            decoration: BoxDecoration(
                              gradient: LinearGradient(
      ...
      child: SizedBox(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            Text(
              amountForOne > 0
                  ? '$delivery-${delivery + 5} mins  |  ${formatCurrency(context, amountForOne)} for one'
                  : '$delivery-${delivery + 5} mins',
              maxLines: 1,
              overflow: TextOverflow.ellipsis,
              style: const TextStyle(
...
class _RestaurantMediaCarouselState extends State<_RestaurantMediaCarousel> {
    ...
    return Stack(
      ...
      children: <Widget>[
        ...
        Positioned(
          left: 12,
          top: 12,
          child: _MenuImageBadge(item: media[_page]),
        ),
        Positioned(
          top: 12,
          right: 12,
  ...
  List<_RestaurantMediaItem> _mediaItems() {
    ...
    add(imageUrl: widget.fallbackImageUrl);
    for (final key in const [
      'menu_items',
      'items',
      'popular_items',
      ...
      for (final raw in list) {
        ...
        add(
          imageUrl: image,
          name: (item['name'] ?? item['title'])?.toString(),
          price: widget.parseDouble(
            item['price'] ?? item['final_price'] ?? item['discounted_price'],
          ),
...
class _MenuImageBadge extends StatelessWidget {
  const _MenuImageBadge({required this.item});

  final _RestaurantMediaItem item;

  @override
  Widget build(BuildContext context) {
    if (item.name.isEmpty && (item.price == null || item.price! <= 0)) {
      return const SizedBox.shrink();
    }
      ...
      child: Container(
        ...
        child: Padding(
          ...
          child: Row(
            children: <Widget>[
              _NavItem(
                asset: AppSvgIcons.home,
                label: 'Home',
                active: currentIndex == 0,
                onTap: () => onTap(0),
              ),
              _NavItem(
                asset: AppSvgIcons.search,
                label: 'Search',
                active: currentIndex == 1,
                onTap: () => onTap(1),
              ),
              _NavItem(
                asset: AppSvgIcons.cart,
                label: 'Cart',
                active: cartActive,
              ...
              _NavItem(
                asset: AppSvgIcons.receipt,
                label: 'Orders',
                active: currentIndex == 3,
                onTap: () => onTap(3),
              ),
              _NavItem(
                asset: AppSvgIcons.user,
                label: 'Profile',
