...
class PaystackWebhookController extends Controller
{
    ...
    {
        $event = $gatewayService->handleWebhook('paystack', $request);
        $payout = Payout::where('gateway_reference_id', $event['reference'])
            ->orWhere('transaction_id', $event['reference'])
            ->orWhere('idempotency_key', $event['reference'])
            ->first();
