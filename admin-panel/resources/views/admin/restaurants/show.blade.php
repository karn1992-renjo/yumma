...
@php
    ...
    $formatCommissionRule = $commissionRule['type'] === 'fixed'
        ? $currencySymbol . number_format($commissionRule['value'], $currencyDecimals)
...
<div class="table-card mb-4">
    <div class="card-header bg-transparent d-flex justify-content-between align-items-center">
        <h5 class="mb-0 fw-bold"><i class="fas fa-chart-line me-2 text-primary"></i> Restaurant Financial Summary</h5>
        <span class="badge bg-light text-dark">{{ $commissionRule['source'] }}: {{ $formatCommissionRule }}</span>
    </div>
    <div class="p-4">
        <div class="row g-4">
            <div class="col-md-3"><div class="small text-muted">Delivered Orders</div><div class="h5 mb-0">{{ number_format((int) ($financialSummary['delivered_orders'] ?? 0)) }}</div></div>
            <div class="col-md-3"><div class="small text-muted">Food Subtotal</div><div class="h5 mb-0">{{ $currencySymbol }}{{ number_format((float) ($financialSummary['food_subtotal'] ?? 0), $currencyDecimals) }}</div></div>
            <div class="col-md-3"><div class="small text-muted">Restaurant Commission</div><div class="h5 mb-0 text-danger">{{ $currencySymbol }}{{ number_format((float) ($financialSummary['restaurant_commission'] ?? 0), $currencyDecimals) }}</div></div>
            <div class="col-md-3"><div class="small text-muted">Net Restaurant Earning</div><div class="h5 mb-0 text-success">{{ $currencySymbol }}{{ number_format((float) ($financialSummary['restaurant_earning'] ?? 0), $currencyDecimals) }}</div></div>
        </div>
        <hr>
        <div class="row g-4">
            <div class="col-md-3"><div class="small text-muted">GST on Commission</div><div class="fw-semibold">{{ $currencySymbol }}{{ number_format((float) ($financialSummary['gst_on_commission'] ?? 0), $currencyDecimals) }}</div></div>
            <div class="col-md-3"><div class="small text-muted">Gateway Fees</div><div class="fw-semibold">{{ $currencySymbol }}{{ number_format((float) ($financialSummary['gateway_fees'] ?? 0), $currencyDecimals) }}</div></div>
            <div class="col-md-3"><div class="small text-muted">Pending Restaurant Payout</div><div class="fw-semibold text-warning">{{ $currencySymbol }}{{ number_format((float) ($financialSummary['pending_earning'] ?? 0), $currencyDecimals) }}</div></div>
            <div class="col-md-3"><div class="small text-muted">Released Restaurant Payout</div><div class="fw-semibold text-success">{{ $currencySymbol }}{{ number_format((float) ($financialSummary['released_earning'] ?? 0), $currencyDecimals) }}</div></div>
            <div class="col-md-3"><div class="small text-muted">Branch Commission</div><div class="fw-semibold">{{ $currencySymbol }}{{ number_format((float) ($financialSummary['branch_commission'] ?? 0), $currencyDecimals) }}</div></div>
            <div class="col-md-3"><div class="small text-muted">Admin Earning</div><div class="fw-semibold">{{ $currencySymbol }}{{ number_format((float) ($financialSummary['admin_earning'] ?? 0), $currencyDecimals) }}</div></div>
            <div class="col-md-3"><div class="small text-muted">Customer Platform Charges</div><div class="fw-semibold">{{ $currencySymbol }}{{ number_format((float) ($financialSummary['platform_charges'] ?? 0), $currencyDecimals) }}</div></div>
                ...
                <table class="table table-hover mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $currencySymbol }}{{ number_format((float) $order->platform_commission, $currencyDecimals) }}</td>
                            <td>{{ $currencySymbol }}{{ number_format((float) $order->gst_on_commission, $currencyDecimals) }}</td>
                            <td>{{ $currencySymbol }}{{ number_format((float) $order->payment_gateway_fee, $currencyDecimals) }}</td>
                            <td class="fw-semibold text-success">{{ $currencySymbol }}{{ number_format((float) $order->restaurant_earning, $currencyDecimals) }}</td>
                            <td>
                                <span class="badge 
