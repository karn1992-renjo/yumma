...
<section class="bf-head">
    <div class="d-flex justify-content-between align-items-center gap-3 flex-wrap">
        <div>
            <h1>Add Banner</h1>
            <p>Create image or Lottie JSON banners for the customer app and home sections.</p>
        </div>
        <a href="{{ route('admin.banners.index') }}" class="btn btn-light border"><i class="fas fa-arrow-left me-2"></i>Back</a>
    </div>
</section>

<form action="{{ route('admin.banners.store') }}" method="POST" enctype="multipart/form-data" class="bf-shell">
    @csrf
        ...
        <div class="bf-body">
            <div class="row g-3">
                <div class="col-md-8">
                    ...
                    <input type="text" name="title" id="bannerTitle" class="form-control @error('title') is-invalid @enderror" value="{{ old('title') }}" required>
                ...
                <div class="col-12">
                    ...
                    <textarea name="description" id="bannerDescription" class="form-control @error('description') is-invalid @enderror" rows="3">{{ old('description') }}</textarea>
                    @error('description') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-12">
                    <label class="form-label fw-bold">Banner Media <span class="text-danger">*</span></label>
                    <input type="file" name="image" id="bannerMedia" class="form-control @error('image') is-invalid @enderror" accept="image/*,.json,application/json" required>
                    <div class="bf-muted mt-1">Upload JPG, PNG, WebP, GIF, or Lottie JSON. Landscape artwork works best in the customer app.</div>
                ...
                <div class="col-md-6">
                    ...
                    <select name="layout_mode" id="layoutMode" class="form-select @error('layout_mode') is-invalid @enderror" required>
                        <option value="text_image" @selected(old('layout_mode', $defaultLayoutMode ?? 'text_image') === 'text_image')>Text + Media</option>
                        <option value="full_image" @selected(old('layout_mode', $defaultLayoutMode ?? '') === 'full_image')>Full Media</option>
                        <option value="promo_card" @selected(old('layout_mode', $defaultLayoutMode ?? '') === 'promo_card')>Promo Widget (Coupon Style)</option>
                    </select>
                    <div class="bf-muted mt-1">Promo Widget shows title, subtitle, a CTA button, and a torn-card image panel with an optional badge/logo &mdash; ideal for "Biryani starting at &#8377;149" style widgets.</div>
                ...
                <div class="col-md-6" data-layout-field="text_image">
                    ...
                    <input type="range" name="image_ratio" id="imageRatio" class="form-range" min="35" max="70" value="{{ old('image_ratio', 46) }}">
                    <div class="bf-muted"><span id="imageRatioValue">{{ old('image_ratio', 46) }}</span>% media area in Text + Media mode.</div>
                ...
                <div class="col-md-6 d-none" data-layout-field="promo_card">
                    ...
                    <input type="text" name="cta_label" id="ctaLabel" class="form-control @error('cta_label') is-invalid @enderror" value="{{ old('cta_label', 'Order Now') }}" maxlength="40" placeholder="ORDER NOW">
                ...
                <div class="col-md-6 d-none" data-layout-field="promo_card">
                    ...
                    <div class="bf-muted mt-1">Small circular logo overlaid on the image corner (optional).</div>
                    @error('badge_image') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-6">
                    <label class="form-label fw-bold">Link URL</label>
                    <input type="url" name="link" class="form-control @error('link') is-invalid @enderror" value="{{ old('link') }}" placeholder="https://example.com/promotion">
    ...
    <aside class="bf-card">
        ...
        <div class="bf-body">
            <div id="bannerPreview" class="bf-preview d-flex align-items-center justify-content-center text-muted">
                <div class="text-center">
                    <i class="fas fa-image fa-3x mb-2"></i>
                    <div class="bf-muted">Upload media to preview.</div>
                </div>
            </div>
            <div class="row g-3 mt-2">
                <div class="col-12">
                    ...
                    <select name="redirect_type" id="redirectType" class="form-select @error('redirect_type') is-invalid @enderror">
                        ...
                        <option value="category" @selected(old('redirect_type') === 'category')>Category</option>
                        <option value="restaurant" @selected(old('redirect_type') === 'restaurant')>Restaurant</option>
                        <option value="menu_item" @selected(old('redirect_type') === 'menu_item')>Menu Item</option>
                    </select>
                ...
                <div class="col-12 redirect-target" data-target="category">
                    ...
                    <select name="redirect_category_id" class="form-select @error('redirect_category_id') is-invalid @enderror">
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" @selected((string) old('redirect_category_id') === (string) $category->id)>{{ $category->name }}{{ $category->restaurant ? ' - '.$category->restaurant->name : '' }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-12 redirect-target" data-target="restaurant">
                    ...
                    <select name="redirect_restaurant_id" class="form-select @error('redirect_restaurant_id') is-invalid @enderror">
                        ...
                        @foreach($restaurants as $restaurant)
                            <option value="{{ $restaurant->id }}" @selected((string) old('redirect_restaurant_id') === (string) $restaurant->id)>{{ $restaurant->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-12 redirect-target" data-target="menu_item">
                    ...
                    <select name="redirect_menu_item_id" class="form-select @error('redirect_menu_item_id') is-invalid @enderror">
                        ...
                        @foreach($menuItems as $menuItem)
                            <option value="{{ $menuItem->id }}" @selected((string) old('redirect_menu_item_id') === (string) $menuItem->id)>{{ $menuItem->name }}{{ $menuItem->restaurant ? ' - '.$menuItem->restaurant->name : '' }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-12 d-grid gap-2">
                    <button type="submit" class="btn btn-primary fw-bold"><i class="fas fa-save me-2"></i>Create Banner</button>
...
function renderPreview() {
    ...
    if (!mediaDataUrl && !lottieAnimationData) {
    ...
    if (layoutModeInput.value === 'full_image') {
        preview.className = 'bf-preview';
        preview.innerHTML = `<img src="${mediaDataUrl}" alt="">`;
        return;
    }
        ...
        preview.innerHTML = `
            <div class="d-flex h-100" style="background:#fff;">
                ...
                <div style="width:48%;position:relative;background:linear-gradient(135deg,#b91c1c,#7f1d1d);clip-path:polygon(12% 0,100% 0,100% 100%,12% 100%,18% 82%,10% 64%,20% 46%,8% 28%,16% 12%);">
                    <img src="${mediaDataUrl}" style="width:100%;height:100%;object-fit:cover;opacity:.92;" alt="">
                </div>
            </div>`;
        return;
    }
    ...
    preview.innerHTML = `
        <div class="d-flex h-100">
            ...
            <div style="width:${ratio}%;" class="p-2 d-flex align-items-center">
                <img src="${mediaDataUrl}" style="width:100%;height:100%;object-fit:contain;" alt="">
            </div>
        </div>`;
}
...
mediaInput?.addEventListener('change', (event) => {
    ...
    const reader = new FileReader();
    reader.onload = (readerEvent) => {
        mediaDataUrl = readerEvent.target.result;
        renderPreview();
    };
