...
/// Funds a restaurant's ad wallet (separate from the personal earnings
/// wallet) via the same Razorpay/Stripe/Cashfree flow as the customer app's
/// wallet top-up, posted against the /restaurant/ads/wallet/top-up*
/// endpoints instead.
class AdWalletTopUpService {
  AdWalletTopUpService({
  ...
  Future<void> start({
    ...
    final response = await _api.post(ApiConstants.restaurantAdsWalletTopUp, data: {
      'amount': amount,
      'payment_method': provider,
    });

    if (response['success'] != true) {
      throw Exception(response['message'] ?? 'Unable to start ad wallet top-up');
    }
    ...
    _pendingRechargeId = int.tryParse('${data['recharge_id'] ?? ''}');

    if (_pendingRechargeId == null) {
      throw Exception('Ad wallet top-up session missing from server');
    }
  ...
  Future<void> _verify(Map<String, dynamic> payload) async {
    ...
    if (rechargeId == null) {
      onFailure('Ad wallet top-up session expired');
      return;
    }

    try {
      final response = await _api.post(ApiConstants.restaurantAdsWalletTopUpVerify, data: {
      ...
      onFailure(
        _normalizeErrorMessage(
          ...
          fallback: 'Ad wallet top-up verification failed',
        ),
      );
    } catch (e) {
      onFailure(
